...
// 生成子订单
const generateSubOrders = (parentId: string, quantity: number, videoInfo: any) => {
  const subOrders = [];
  // 从现有数据中获取最大的子订单ID号
  ...
  for (let i = 1; i <= quantity; i++) {
    subOrders.push({
      ...
      screenshotUrl: "",
      sharingDetails: {
        platform: videoInfo.platform || "wechat",
        sharingMethod: "direct_share",
        requiredAudience: videoInfo.requiredAudience || "friends"
      }
    });
  }
  return subOrders;
...
// 处理截止时间参数
const processDeadline = (timeOption: string) => {
  const hours = parseInt(timeOption);
  if (isNaN(hours)) {
    return new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(); // 默认24小时
  }
...
export async function POST(request: Request) {
  ...
  try {
    console.log('=== Video Send API Called ===');
    ...
    // 从请求体中获取任务信息（处理FormData）
    const formData = await request.formData();
    console.log('FormData keys:', Array.from(formData.keys()));
    
    // 提取参数
    const taskId = formData.get('taskId') as string || 'video_send';
    const taskTitle = formData.get('taskTitle') as string || '视频推送';
    const basePrice = formData.get('taskPrice') as string;
    const quantity = formData.get('quantity') as string || '1';
    const videoRequirements = formData.get('videoRequirements') as string || '';
    const receiverInfo = formData.get('receiverInfo') as string || '';
    const contactInfo = formData.get('contactInfo') as string || '';
    const deadlineOption = formData.get('deadline') as string || '24';
    
    // 计算总费用并进行类型转换和验证
    const basePriceNum = parseFloat(basePrice || '0');
    const quantityNum = parseInt(quantity || '1');
    const totalCost = basePriceNum * quantityNum;
    
    // 验证数量参数
    if (isNaN(quantityNum) || quantityNum <= 0) {
      return NextResponse.json(
        { success: false, message: '推送数量必须是正整数' },
        { status: 400 }
      );
    }
    ...
    const videoInfo = {
      platform: 'wechat',
      videoUrl: '',
      requiredAudience: 'friends'
    };
    
    // 使用videoRequirements作为targetAudience
    const targetAudience = videoRequirements;
    const specialRequirements = receiverInfo;
    
    // 验证必需参数
    if (!taskId || !taskTitle || !videoInfo || !targetAudience) {
      console.log('缺少必要参数:', { taskId, taskTitle, videoInfo, targetAudience });
      return NextResponse.json(
    ...
    if (!targetAudience.trim()) {
      return NextResponse.json(
        { success: false, message: '请输入视频需求描述' },
        { status: 400 }
      );
    }
    ...
    // 处理截止时间
    const processedDeadline = processDeadline(deadlineOption);
    ...
    const newOrder = {
      ...
      taskRequirements: targetAudience + (specialRequirements ? `\n\n特定要求：${specialRequirements}` : ''),
      publishTime: new Date().toISOString(),
      deadline: processedDeadline,
      needImageComment: true, // 视频推送通常需要截图
      taskType: "video_send",
      title: taskTitle,
      videoInfo: videoInfo,
      subOrders: generateSubOrders(newOrderId, quantityNum, videoInfo) // 生成对应数量的子订单
    };
    ...
    // 添加交易记录
    addTransactionRecord(currentUserId, newOrderId, totalCost, 'expense', '发布视频推送任务');
    ...
    console.error('Video send creation error:', error);
